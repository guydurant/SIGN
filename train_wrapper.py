...
from preprocess_pdbbind import pocket_atom_num_from_mol2, gen_pocket_graph, dist_filter, distance_matrix, cons_lig_pock_graph_with_spatial_context
...
def pocket_atom_num_from_mol2(key):
    n = 0
    with open(f'temp_files/{key}_pocket.mol2') as f:
        for line in f:
...
def gen_feature(protein_file, ligand_file, key, featurizer):
    charge_idx = featurizer.FEATURE_NAMES.index('partialcharge')
    # calculate_charges(ligand_file)
    # ligand = next(pybel.readfile('mol2', f'temp_files/{ligand_file.split("/")[-1].split(".")[0]}_charged.mol2'))
...
def pairwise_atomic_types(protein_files, ligand_files, keys, processed_dict, atom_types, atom_types_):
...
def process_dataset(csv_file, data_dir, cutoff):
    ...
    processed_dict = {}
    for i in tqdm(range(len(keys))):
        processed_dict[keys[i]] = gen_feature(protein_files[i], ligand_files[i], keys[i], featurizer)
        os.system(f'rm temp_files/{keys[i]}*')

    # interaction features
    processed_dict = pairwise_atomic_types(protein_files, ligand_files, keys, processed_dict, atom_types, atom_types_)
    ...
    with open(os.path.join(f'temp_features/{csv_file.split("/")[-1].split(".")[0]}' + '_features.pkl'), 'wb') as f:
